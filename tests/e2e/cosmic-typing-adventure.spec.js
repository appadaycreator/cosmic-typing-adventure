...
const SVC_URL = "https://appadaycreator.github.io/cosmic-typing-adventure/";
const SVC_NAME = "タイピングゲーム";

test.describe("タイピングゲーム - E2Eテスト", () => {

  test("ページ基本: タイトル・h1・canonical", async ({ page }) => {
    await page.goto(SVC_URL);
    const title = await page.title();
    expect(title.length).toBeGreaterThan(15);
    expect(title.toLowerCase()).toMatch(/タイピング/i);
    const canonical = await page.locator('link[rel="canonical"]').getAttribute("href");
    expect(canonical).toContain("cosmic-typing-adventure");
  ...
  test("SEO: sitemap.xml・robots.txt が存在する", async ({ page }) => {
    const sitemapResp = await page.request.get("https://appadaycreator.github.io/cosmic-typing-adventure/sitemap.xml");
    expect([200, 301, 302]).toContain(sitemapResp.status());
    const robotsResp = await page.request.get("https://appadaycreator.github.io/cosmic-typing-adventure/robots.txt");
  ...
  test("プライバシー・利用規約: ページが存在する", async ({ page }) => {
    const privResp = await page.request.get("https://appadaycreator.github.io/cosmic-typing-adventure/privacy-policy.html");
    const termsResp = await page.request.get("https://appadaycreator.github.io/cosmic-typing-adventure/terms.html");
